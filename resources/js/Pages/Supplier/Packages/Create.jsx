...
export default function Create({ services = [], teams = [], selectedTeamId = null, categories = [] }) {
    const [imagePreview, setImagePreview] = useState(null);
    const [serviceTab, setServiceTab] = useState('my_services'); // 'my_services' | 'team_services'
    const fileInputRef = useRef(null);

    const { data, setData, post, processing, errors } = useForm({
        name: '',
        team_id: selectedTeamId ? String(selectedTeamId) : '',
        event_category_id: categories.length > 0 ? String(categories[0].id) : '',
        price: '',
        description: '',
        inclusions: '',
        image: null,
    ...
    // Calculate selected services list and total individual value
    ...
    return (
        <DashboardLayout>
            <Head title="Add New Package - Supplier Portal" />
